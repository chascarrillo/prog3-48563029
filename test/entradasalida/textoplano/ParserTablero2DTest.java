...
public class ParserTablero2DTest {
	...
	public void testLeeTablero2D1() {
		...
		try {
			...
			if (!(c2.equals(new Coordenada2D(2,2)))) fail("La dimensión debe de ser "+new Coordenada2D(2,2).toString()+" y muestra "+c2.toString());
			else {
				...
				for (int i=0; i<dimy; i++) {
					...
					assertEquals("Comparacion tableros2D "+c2.toString(),EstadoCelda.MUERTA,tab.getCelda(new Coordenada2D(j,i)));
					}
				}
			}
		...
		try {
			...
			if (!(c2.equals(new Coordenada2D(2,2)))) fail("La dimensión debe de ser "+new Coordenada2D(2,2).toString()+" y muestra "+c2.toString());
				...
				for (int i=0; i<dimy; i++) {
					for (int j=0; j<dimx; j++) {
					c2 = new Coordenada2D(j,i);				
					assertEquals("Comparacion tableros2D "+c2.toString(),EstadoCelda.VIVA,tab.getCelda(new Coordenada2D(j,i)));
					}
				}
			}
	...
	public void testLeeTablero2D3() {
		...
		try {
			...
			if (!(c2.equals(new Coordenada2D(5,5)))) fail("La dimensión debe de ser "+new Coordenada2D(5,5).toString()+" y muestra "+c2.toString());
			else {
				int dimx = ((Coordenada2D)tab.getDimensiones()).getX();
				int dimy = ((Coordenada2D)tab.getDimensiones()).getY();
				for (int i=0; i<dimy; i++) {
