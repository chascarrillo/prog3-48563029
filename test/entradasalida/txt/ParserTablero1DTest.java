...
public class ParserTablero1DTest {
	...
	public void testLeeTablero1D1() {	
		try {
			...
			if (dim != 1) fail("La dimensión debe de ser 1 y muestra "+dim);
			else {
				for (int i=0; i<dim; i++) {
					assertEquals("Comparacion tableros1D "+i,stab.getCelda(new Coordenada1D(i)),tab.getCelda(new Coordenada1D(i)));
				}
			}
	...
	public void testLeeTablero1D3() {
		...
		try {
			...
			int dim = ((Coordenada1D)tab.getDimensiones()).getX();
			if (dim != st.length()) fail("La dimensión debe de ser "+st.length()+" y muestra "+dim);
